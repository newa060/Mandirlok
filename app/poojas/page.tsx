  ...
  return (
    <div className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border border-gray-100 flex flex-col">
      ...
      <div className="relative h-52 overflow-hidden bg-gradient-to-br from-orange-800/60 to-red-900/70 flex items-center justify-center">
        {puja.images?.[0] ? (
          <img src={puja.images[0]} alt={puja.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
        ) : (
          <>
            <div
      ...
      <div className="p-5 flex flex-col flex-1">
        <h3 className="font-bold text-gray-900 text-base mb-2 line-clamp-2 leading-snug">{puja.name}</h3>
        ...
        <p className="text-xs text-gray-600 mb-4 line-clamp-2 leading-relaxed flex-1">{puja.description}</p>
      ...
      <div className="relative z-10 h-full flex items-center">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
          <div className="flex flex-wrap gap-3 mb-4">
            <span className="bg-orange-500/90 text-white text-xs font-bold px-3 py-1.5 rounded-full">Divine Blessings through Puja</span>
            <span className="bg-white/10 text-white text-xs px-3 py-1.5 rounded-full backdrop-blur-sm">Vedic Rituals by Expert Pandits</span>
            <span className="bg-white/10 text-white text-xs px-3 py-1.5 rounded-full backdrop-blur-sm">Video in 24–48 Hours</span>
          </div>
          <h1 className="text-3xl md:text-4xl font-bold text-white mb-3">Mandirlok Special Pujas</h1>
          <p className="text-white/80 text-base max-w-xl leading-relaxed">
            Get authentic Vedic pujas performed at India's most powerful temples on your behalf.
          </p>
        </div>
      </div>
    </section>
...
function HowItWorksBanner() {
  const steps = [
    { icon: "🛕", title: "Choose Puja",     desc: "Select from 200+ authentic pujas across India's sacred temples" },
    { icon: "✍️", title: "Enter Your Name", desc: "Panditji will chant your name in the Sankalp during the puja" },
    { icon: "💳", title: "Secure Payment",  desc: "Pay securely via UPI, card or net banking through Razorpay" },
    { icon: "📹", title: "Get Puja Video",  desc: "Receive the video proof of your completed puja within 24–48 hrs" },
  ];
  return (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {steps.map((s, i) => (
            <div key={i} className="flex items-start gap-3">
              <div className="w-10 h-10 rounded-xl bg-orange-100 flex items-center justify-center text-xl shrink-0">{s.icon}</div>
              <div>
              ...
              {FILTERS.map((f) => (
                <button
                  ...
                  className={`shrink-0 px-4 py-1.5 rounded-full text-sm font-medium transition-all duration-200 whitespace-nowrap ${
                    activeFilter === f ? "bg-orange-500 text-white shadow-md" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                  }`}
                >
                  {f}
                </button>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {error && (
            <div className="text-center py-10">
              ...
              <button onClick={() => { setSearch(""); setActiveFilter("All Pujas"); }} className="bg-orange-500 text-white px-6 py-2.5 rounded-full text-sm font-semibold">
                Try Again
              </button>
            </div>
          )}
          ...
          {!loading && !error && poojas.length === 0 && (
            <div className="text-center py-20">
              ...
              <h3 className="text-xl font-bold text-gray-700 mb-2">No pujas found</h3>
